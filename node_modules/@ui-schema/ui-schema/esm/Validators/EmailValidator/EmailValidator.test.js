...
describe('emailValidator', () => {
  ...
  'demo)dee@example.org', false]])('emailValidator.handle(%s): %j', (email, isValid) => {
    // @ts-ignore
  ...
  test.each([[OrderedMap({
    ...
    type: 'string',
    format: 'email'
  }), 'some-text', true], [OrderedMap({
    type: 'string',
    format: 'email'
  }), undefined, false], [OrderedMap({
    type: 'string',
    format: 'email'
  }), 0, false], [OrderedMap({
    type: 'integer',
    format: 'email'
  }), 'some-text', false], [OrderedMap({
    type: 'string',
    format: 'emails'
  }), 'some-text', false], [OrderedMap({
    type: ['string'],
    ...
    type: ['string', 'integer'],
    format: 'email'
  }), 10, false]])('emailValidator.should(%s): %j', (schema, email, expected) => {
    // @ts-ignore
